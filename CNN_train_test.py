...
"""
Define a simple CNN with all kind of layers.
"""
class Net:
    def __init__(self):
        # input: 28x28
...
# # Step 8: Create random subset samples of MNIST
...
from precode import *
print('Loading data......')
sub_train_images, sub_train_labels, sub_test_images, sub_test_labels = init_subset('0038') 
...
def evaluate(net, images, labels):
    ...
    for batch_index in range(0, images.shape[0], batch_size):
        ...
        ### forward propagation to get the values from the net
        for l in range(net.lay_num):    
            output = net.layers[l].forward(x)  
            x = output
        ### calculate the loss
        loss += cross_entropy(output, y)
        ### if the labels match, then increase acc by 1
        if np.argmax(output) == np.argmax(y):
            acc += 1
    ### normalized by sample number    
